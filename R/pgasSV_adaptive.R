####################################################################################################################### 
#   PGAS for SV model with Adaptive MCMC.  Input (nmcmc, burnin, y, phi_init, q_init, npart, mcmseed)    
...
pgasSV_binorm_ada = function(nmcmc, burnin, y, init, hyper, sigma_MH, npart, lambda, alpha.tar, parms_2 = TRUE, mcmseed){
  # Input:
  ...
  #   sigma_MH - initial value of covariance matrix for Random Walk Metropolis Hastings
  #   lambda - initial value of lambda
  #   alpha.tar - target acceptance rate
  ...
  lambda.trace = rep(0,numMCMC)
  gamma = rep(0.5,numMCMC)
  g_diff.all = rep(0,numMCMC)
  sigma.all = list(NA)
  mu.all = list(NA)
  ...
  mu.all[[1]] = mu_MH
  sigma.all[[1]] = sigma_MH
  

  # Initialize the state by running a PF
  if (parms_2){
    
    u = cpf_as_sv(y, phi[1], q[1], 0, N,  X[1,])    
    ...
    u = cpf_as_sv(y, phi[1], q[1], mu[1], N,  X[1,])    
    particles = u$x           # returned particles
    w = u$w                   # returned weights
    # Draw J
    J = which( (runif(1)-cumsum(w[,T])) < 0 )[1]
  ...
  if (parms_2){
    ...
    for(k in 2:numMCMC){
      ...
      (parms_star = parms + mvrnorm(1, mu_MH, lambda.trace[k-1]*sigma.all[[k-1]]))
      ...
      g_diff.all[k-1]=g_diff
      parm.new = cbind(phi[k],sqrt(q[k]))
      
      # Update Lambda
      #lambda.trace[k] = lambda
      lambda.trace[k] = exp(log(lambda.trace[k-1]) + gamma[k-1]*(exp(g_diff.all[k-1])-alpha.tar))
      sigma.all[[k]] = sigma.all[[k-1]] + gamma[k-1]*(as.matrix(t(parm.new-mu.all[[k-1]]))%*%(as.matrix(parm.new-mu.all[[k-1]])) - sigma.all[[k-1]])
      mu.all[[k]] = mu.all[[k-1]] + gamma[k-1]*(parm.new - mu.all[[k-1]])
      gamma[k] = 1/(500*k)
      ...
      # Run CPF-AS
      u = cpf_as_sv(y, phi[k], q[k], 0, N, X[k-1,])
      particles = u$x           # returned particles
  ...
  bi = 1:burnin
  acp = (dim(table(phi[-bi]))-1)/nmcmc*100
  time2run10 = proc.time() - ptm
  list(phi=phi[-bi], q=q[-bi], mu=mu[-bi], X=X[-bi,], time=time2run10, acp=acp)
}#end
